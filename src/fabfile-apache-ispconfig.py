...
env.proj_app = real_project_name("mezz")
...
env.domain = env.hosts[0]

env.local_path = '/cygdrive/d/users/dat/workspace/mezz/'
env.extra_opts = '--chmod=D2775,F664'

## my added
env.website_id = conf.get("WEBSITE_ID", None)
env.client_id = conf.get("CLIENT_ID", None)

if env.website_id is None or env.client_id is None:
    print("website_id must be provided")
    exit(1)
    

env.proj_name = conf.get("PROJECT_NAME", env.proj_app)
env.venv_home = conf.get("VIRTUALENV_HOME", "/var/www/clients/%s/%s/home/%s/.virtualenvs" % (env.client_id, env.website_id, env.user))
env.venv_path = join(env.venv_home, env.proj_name)
env.proj_path = "/var/www/clients/%s/%s/home/%s/mezzanine/%s" % (env.client_id, env.website_id, env.user, env.proj_name)
...
if env.deploy_tool == "git":
    env.repo_path = "/var/www/clients/%s/%s/home/%s/git/%s.git" % (env.client_id, env.website_id, env.user, env.proj_name)
else:
...
templates = {
    "apache": {
        "local_path": "deploy/apache-ispconfig-django.template",
        "remote_path": "/etc/apache2/sites-enabled/100-%(domain)s.vhost",
    ...
    "uwsgi": {
        "local_path": "deploy/django-wsgi.template",
        "remote_path": "%(proj_path)s/%(proj_name)s/wsgi.py",
    },
    "settings": {
...
def modsudo(user=env.user):
    """
    Add current user to sudoer group
    """
    run("usermod -G sudo %s" % user)
...
def install():
    ...
    run("echo 'source /usr/local/bin/virtualenvwrapper.sh' >> "
        "/var/www/clients/%s/%s/home/%s/.bashrc" % (env.client_id, env.website_id, env.user))
